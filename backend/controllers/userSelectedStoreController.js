...
export const getSelectedStore = async (req, res) => {
  try {
    ...
    // Always fetch fresh store data from database to ensure all fields (like gstNumber) are included
    let selectedStore = null;
    if (user.selectedStoreId) {
      const numericId = Number(user.selectedStoreId);
      if (Number.isFinite(numericId) && numericId > 0) {
        selectedStore = await getStoreById(numericId);
      }
    }

    // Fallback to user.selectedStore if fresh fetch fails (for backward compatibility)
    if (!selectedStore && user.selectedStore) {
